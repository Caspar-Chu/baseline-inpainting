#!/usr/bin/env python3
"""Single-image LaMa smoke test using cached masks."""
...
from baseline_suite.io import load_image, save_image
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LaMa single-sample smoke test")
    parser.add_argument(
        "--experiment",
        default="missing_patterns",
        choices=["random_pixel_masks", "block_masks", "missing_patterns", "scenes_resolutions"],
    )
    parser.add_argument("--case", default="random", help="Sample case name")
    parser.add_argument(
        "--output",
        default="results/smoke_lama/restored.png",
    ...
    inpainter = create_baseline("lama")
