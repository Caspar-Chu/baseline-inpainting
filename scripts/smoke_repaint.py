#!/usr/bin/env python3
"""Single-image RePaint smoke test using cached masks."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RePaint single-sample smoke test")
    parser.add_argument(
        "--experiment",
    ...
    parser.add_argument(
        "--output",
        default="results/smoke_repaint/restored.png",
        help="Where to save restored image",
    )
    parser.add_argument(
        "--show-progress",
        action="store_true",
        help="Show RePaint diffusion progress bar",
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Use reduced diffusion schedule for quick smoke (not for metrics)",
    )
    ...
    kwargs: dict = {"show_progress": args.show_progress}
    if args.fast:
        from baseline_suite.paths import CONFIGS_DIR

        kwargs["conf_path"] = CONFIGS_DIR / "repaint_places256_fast.yml"

    inpainter = create_baseline("repaint", **kwargs)
